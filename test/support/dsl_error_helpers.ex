defmodule AshTypst.Test.DslError do
  @moduledoc """
  Test helpers for asserting on `Spark.Error.DslError` raised by the AshTypst
  ...
  defmacro assert_dsl_error(expected, do: block) do
    quote do
      {err, captured} =
        AshTypst.Test.DslError.__capture_dsl_error__(fn ->
          unquote(block)
        end)

      actual = AshTypst.Test.DslError.__select_actual__(err, captured, unquote(expected))
      AshTypst.Test.DslError.__match_dsl_error__(actual, unquote(expected))
    end
  end
  ...
  defmacro assert_dsl_warning(expected, do: block) do
    quote do
      ...
      stripped = AshTypst.Test.DslError.extract_warning_body(output)
